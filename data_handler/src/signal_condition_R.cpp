...
#define posY_ws    277.34
#define posZ_ws    75
#define oriY_ws    -95.3413
#define oriP_ws    -2.0475
#define oriR_ws    -105.6926
...
public:
  ...
  SubscribeAndPublish()
  {
    ...
    pub_absPosition    =nr.advertise<geometry_msgs::Point>("r_position",100);
    pub_absOrientation = nr.advertise<geometry_msgs::Point>("r_orientation",100);
    pub_gripper      = nr.advertise<std_msgs::Float64>("r_grip_data",100);

    //Topic you want to subscribe    
    sub_dRPos = nr.subscribe("r_dPosition_raw", 30, &SubscribeAndPublish::rawPosCallback, this);
    sub_dROri = nr.subscribe("r_dOrientation_raw", 30, &SubscribeAndPublish::rawOriCallback, this); 
    sub_ui = nr.subscribe("UI",30, &SubscribeAndPublish::uiCallback, this);
    sub_gripper = nr.subscribe("r_grip_data_raw", 30, &SubscribeAndPublish::gripCallback, this);
...
{
  //Initiate ROS
  ros::init(argc, argv, "signal_condition_node_R");

  //Create an object of class SubscribeAndPublish that will take care of everything
  SubscribeAndPublish signal_condition_node_R;
