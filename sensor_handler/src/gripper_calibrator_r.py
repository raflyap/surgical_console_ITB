#!/usr/bin/env python
'''
    Gripper data handler Node Right
    Update = 26 8 2020
'''
import rospy
...
pub_grip_limit = rospy.Publisher('r_gripper_limit', gripperLimit, queue_size= 200)
...
def callback_gripState(data):
    ...
    if data.calibGrip == 0:
        ...
        r.sleep() 
    elif data.calibGrip != 0 and data.calibArm == 0:
        r = rospy.Rate(300)
        if data.calibGrip == 1:
            ...
            with  open('/home/epione/console_ws/src/save_data/r_gripper_dat.fla', 'w') as files:                 
...
def gripper_handler():
    ...
    rospy.init_node('gripper_handler_right', anonymous = True)
    if load == 0:
        #load data
        list_gripperLim = list()
        r = rospy.Rate(300)
        fhandle = open('/home/epione/console_ws/src/save_data/r_gripper_dat.fla', 'r')
        for line in fhandle:
    ...
    rospy.Subscriber('r_arm_data', sensorData, callbackGripper)
    rospy.spin()
